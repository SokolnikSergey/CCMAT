...
class SignalsBinder(QObject):
    ...
    def snapping_view_operator_signals(self):
        if self.__view_operator is not None:
            self.__view_operator.inform_me_recieved.connect(self.__interrupt_convertor.make_inform_me_btc_request)
            self.__view_operator.buy_recieved.connect(self.__interrupt_convertor.make_withdraw_btc_request)

    def snapping_bill_acceptor_signals(self):
        self.__bill_acceptor.money_received.connect(
